...
def main():
    ...
    if args.verbose:
        ...
        print(" Tokens processados (use --verbose para detalhes).")
    ...
    if erros_sintaticos:
        print(f"\n ERRO: {len(erros_sintaticos)} erro(s) sintático(s) encontrado(s).")
    ...
    if ast is None:
        print("  ERRO: Falha na análise sintática (AST nula).")
        sys.exit(1)
    ...
    if avisos:
        print(f"\n AVISO: {len(avisos)} aviso(s):")
        for a in avisos:
            print(f"   {a}")

    if erros_semanticos:
        print(f"\n ERRO: {len(erros_semanticos)} erro(s) semântico(s):")
